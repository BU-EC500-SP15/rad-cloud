...
class crun_hpc_mosix(crun_hpc):
    ...
    def blockOnChild(self):
        ...
        while b_jobsInQueue:
            ...
            time.sleep(5)
            for jID in self._jobID_list:
                shell('mosq listall | grep %s | wc -l' % jID)
                b_jobsInQueue = b_jobsInQueue or int(shell.stdout().strip())
...
if __name__ == '__main__':
    ...
    if args.scheduler:
        try:
            shell = eval(args.scheduler + '(remoteUser=user, remoteHost=host, emailUser=mail, schedulerStdOutDir=out, schedulerStdErrDir=err)')
        except NameError:
